...
# code: https://github.com/clear-code-projects/FlappyBird_Python
...
# Init the game, set the screen and clock
...
game_active = os.truncate
score = 0
high_score = 0
can_score = True

# load the background, convert it for pygame and scale it to the image
bg_surface = pygame.transform.scale(pygame.image.load(
    os.path.join("assets", "background-day.png")).convert(), (WIDTH, HEIGHT))

# load the base surface and scale it proportional to the width
...
bird_downflap = pygame.transform.scale2x(pygame.image.load(
    os.path.join("assets", "bluebird-downflap.png")).convert_alpha())
bird_midflap = pygame.transform.scale2x(pygame.image.load(
    os.path.join("assets", "bluebird-midflap.png")).convert_alpha())
bird_upflap = pygame.transform.scale2x(pygame.image.load(
    os.path.join("assets", "bluebird-upflap.png")).convert_alpha())
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == SPAWNPIPE:
            # return tuple and add it to the list with "extend" instead of "append"
    ...
    if game_active:
        ...
        # display moving floor: use two floors (onw after the other one)
        # as soon as one floor is over, reset the x axis
    floor_x_pos -= 0.5
    draw_floor()
